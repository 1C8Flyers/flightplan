...
async function fetchTfrGeoJson() {
...
function parseRunwayEndIdent(value) {
...
function extractRunwayEndpointCoordinates(line) {
...
async function fetchRunwaysDataset() {
    ...
    for (const line of lines) {
        ...
        const [rawLeIdent, rawHeIdent] = runwayIdent.split('/').map((value) => value.trim().toUpperCase());
...
async function fetchStationByCandidateCodes(code) {
    const candidates = getAirportCodeCandidates(code);
    for (const candidate of candidates) {
        try {
            const payload = await fetchJson(`https://aviationweather.gov/api/data/stationinfo?ids=${candidate}&format=json`);
            if (!payload.length) {
                continue;
            }
...
function findAirportInDataset(code, airports) {
    const normalized = code.toUpperCase().trim();
    const candidates = getAirportCodeCandidates(normalized);
    return (airports.find((airport) => candidates.includes(airport.ident.toUpperCase()) ||
        ...
        (airport.gpsCode ? candidates.includes(airport.gpsCode.toUpperCase()) : false)) ?? null);
}
...
function resolveWaypointIdentInDatasets(ident, airports, navaids) {
...
function decodeWindGroup(group, altitudeFt) {
    if (!group || group === '//////') {
        return null;
    }
...
async function fetchWeatherFromStationIds(ids) {
    if (!ids.length) {
        return { metar: null, taf: null, sourceStation: null };
    }
    const idList = ids.join(',');
    const [metarPayload, tafPayload] = await Promise.all([
        fetchJson(`https://aviationweather.gov/api/data/metar?ids=${idList}&format=json`),
        fetchJson(`https://aviationweather.gov/api/data/taf?ids=${idList}&format=json`)
    ]);
        ...
        for (const candidate of legCandidates) {
            ...
            if (!existing || candidate.crossTrackNm < existing.closestDistanceNm) {
                byIdent.set(candidate.ident, {
                    ...
                    offRouteDirection: Math.abs(candidate.crossTrackSignedNm) < 0.05
                        ? 'center'
...
async function fetchSectionalCharts() {
    ...
    while (match) {
        ...
        const skip = filename.includes('_TAC') ||
    ...
    const scoreAirport = (chartName, airportName, airportType) => {
...
app.get('/api/frequencies/:icao', async (req, res) => {
    try {
        ...
        const deduped = matches.filter((item, index, array) => array.findIndex((candidate) => candidate.type === item.type &&
            candidate.description === item.description &&
            candidate.frequencyMHz === item.frequencyMHz) === index);
...
app.get('/api/sectionals/geotiff', async (req, res) => {
    try {
        ...
        const tifEntry = zip
            .getEntries()
            .find((entry) => /\.tif(f)?$/i.test(entry.entryName));
        if (!tifEntry) {
...
async function buildSectionalOverlay(source) {
    ...
    const htmEntry = zip
        .getEntries()
        .find((entry) => /\.htm$/i.test(entry.entryName));
    const tfwEntry = zip
        .getEntries()
        .find((entry) => /\.tfw$/i.test(entry.entryName));
    if (!tifEntry) {
...
app.get('/api/winds-aloft', async (req, res) => {
    try {
        ...
        const stationCandidates = stations
            .map((station) => {
            const matchedAirport = airports.find((airport) => airport.iataCode === station.station ||
