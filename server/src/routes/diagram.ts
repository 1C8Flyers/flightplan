...
export type RunwayGeom = {
  ...
  heLat?: number
  heLon?: number
  leTrueHeadingDeg?: number
  heTrueHeadingDeg?: number
...
type Feature<G extends string, P extends Record<string, unknown>> = {
  type: 'Feature'
  geometry: {
    type: G
    coordinates: unknown
...
type FeatureCollection = {
  type: 'FeatureCollection'
  features: Array<Feature<string, Record<string, unknown>>>
}
...
type DiagramDeps = {
  fetchAirportsDataset: () => Promise<AirportRecord[]>
  fetchRunwaysDataset: () => Promise<RunwayRecord[]>
}

const cacheTtlMs = 5 * 60 * 1000
const diagramCache = new Map<string, { loadedAt: number; data: DiagramResponse }>()
...
function runwayId(leIdent: string, heIdent: string) {
  return `${leIdent}/${heIdent}`
}

function toRunwayGeom(runway: RunwayRecord): RunwayGeom {
  return {
...
function resolveRunwayEndpoints(runway: RunwayGeom, airport: AirportRecord) {
  ...
  return {
    ...
    fidelity: 'estimated' as const,
    headingDeg
  }
}

function buildRunwayLabels(
  runway: RunwayGeom,
  leLon: number,
  leLat: number,
  heLon: number,
  ...
  const leLabel: Feature<'Point', Record<string, unknown>> = {
    type: 'Feature',
    geometry: {
      type: 'Point',
  ...
  const heLabel: Feature<'Point', Record<string, unknown>> = {
    type: 'Feature',
    geometry: {
      type: 'Point',
...
function buildClosedRunwayOverlay(runwayPolygon: [number, number][], runway: RunwayGeom) {
  ...
  ] as Array<Feature<'LineString', Record<string, unknown>>>
}

function buildDisplacedThresholdOverlay(
  runway: RunwayGeom,
  leLon: number,
  leLat: number,
  heLon: number,
  ...
  const overlays: Array<Feature<'LineString', Record<string, unknown>>> = []
  const leMerc = lonLatToMercator(leLon, leLat)
  const heMerc = lonLatToMercator(heLon, heLat)
  const runwayVector = normalizeVector({ x: heMerc.x - leMerc.x, y: heMerc.y - leMerc.y })
  ...
  const addThreshold = (thresholdFt: number | undefined, start: typeof leMerc, direction: number, end: 'LE' | 'HE') => {
...
function buildSchematic(airport: AirportRecord, runwayFeatures: FeatureCollection) {
  ...
  const runwayCoords = longestRunway.geometry.type === 'Polygon'
    ? (longestRunway.geometry.coordinates as [number, number][][])[0]
    : null
  ...
  const runwayMidpoints = runwayFeatures.features
    ...
    .filter((value): value is { id: string; midpoint: { x: number; y: number } } => Boolean(value))
    .slice(0, 3)
...
function buildDiagram(airport: AirportRecord, runwayRecords: RunwayRecord[], includeSchematic: boolean): DiagramResponse {
  const runways: FeatureCollection = { type: 'FeatureCollection', features: [] }
  const runwayLabels: FeatureCollection = { type: 'FeatureCollection', features: [] }
  ...
  for (const runwayRecord of airportRunways) {
    ...
    const runwayPolygon = buildRunwayPolygon(
      resolved.le.lon,
      resolved.le.lat,
      resolved.he.lon,
      resolved.he.lat,
    ...
    const labels = buildRunwayLabels(
      runway,
      resolved.le.lon,
      resolved.le.lat,
      resolved.he.lon,
    ...
    overlays.features.push(
      ...buildDisplacedThresholdOverlay(runway, resolved.le.lon, resolved.le.lat, resolved.he.lon, resolved.he.lat)
    )
  }
...
export function registerDiagramRoutes(app: Express, deps: DiagramDeps) {
