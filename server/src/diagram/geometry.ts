...
export function lonLatToMercator(lon: number, lat: number): Point2D {
...
export function metersFromFeet(valueFt: number) {
  return valueFt * 0.3048
}

export function projectFromHeading(originLon: number, originLat: number, headingDeg: number, distanceMeters: number) {
...
export function buildRunwayPolygon(
  leLon: number,
  leLat: number,
  heLon: number,
  heLat: number,
...
export function polygonWidthMeters(polygon: [number, number][]) {
  if (polygon.length < 4) {
    return 0
  }
